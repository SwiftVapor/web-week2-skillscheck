...
let doubled = myNumbers.map(function(ele){
	return ele * 2;
});
...
let filtered = myNumbers.filter(function(ele){
	return ele > 100;
});
...
let total = myNumbers.reduce(function(acc,ele){
	return acc+ele;
})
...
let myNumbersIndex = []
myNumbers.forEach (function(ele, ind, arr){
...
let forTheLoveOfGeorge = notGeorge.map((ele)=>{
	return ele='George'
})
...
// Code Here
let awsomeness = people.map((ele)=>{
	return ele.awesomeLevel;
})

let totallyAwesome = awsomeness.reduce((acc, ele, ind, arr)=>{
	return ele + acc
});
